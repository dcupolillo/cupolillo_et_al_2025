// ImageJ Macro: ZsGreen+ Neuron Position Mapping in CA3
//
// This macro extracts the normalized positions of ZsGreen+ neurons along the 
// CA3 axis in hippocampal sections from fluorescence microscopy images.
//
...
//
// Output:
//   - CSV file with columns: ID, Treatment, Slide, Section, CA3_axis_length, 
//     followed by normalized X and Y positions of each neuron (0-1 scale)
//
...
output_path = output_dir + "neuron_position.csv";

// Set polyline width
run("Line Width...", "line=165");

// Assumes specific file organization
...
for (j = 0; j < exp_list.length; j++) {
    ...
    for (k = 0; k < mouse_list.length; k++) {
        ...
        for (i = 0; i < file_list.length; i++) {
            ...
            lines[0] = mouse_id;
            lines[1] = treatment;
            lines[2] = slide_number;
            lines[3] = section_id;
            
            // Initialize workspace
            run("Close All");
            roiManager("Reset");
            run("Set Measurements...", "area centroid area_fraction display redirect=None decimal=3");
            run("Clear Results");
            ...
            if (lengthOf(info) == 4) {
                ...
                // Concatenate results
                lines = Array.concat(lines, neuron_position_x, neuron_position_y);
                Array.print(lines);
            }
        }
    }
